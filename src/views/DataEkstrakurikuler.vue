    ...
    <v-row>
      <v-col class="text-right">
        <v-btn
          ...
          @click="dialog = !dialog"
        >
          <v-icon left>mdi-plus</v-icon>Tambah Ekstrakurikuler
        </v-btn>
      </v-col>
    </v-row>
    ...
    >
      <v-card>
        <v-toolbar dark color="primary">
          ...
          <v-toolbar-title>Tambah Ekstrakurikuler</v-toolbar-title>
          <v-spacer></v-spacer>
          <v-toolbar-items>
            <v-btn
              dark
  ...
  methods: {
    ...
    save() {
      ...
      if (this.$refs.form.validate() === true) {
        this.$http
          .post("/api/ekstrakurikuler", this.editedItem)
          .then((r) => {
            this.snackbar = {
              show: true,
    ...
    processingDelete(item) {
      this.$http
        .delete(`/api/ekstrakurikuler/${item.id}`)
        .then((r) => {
          this.snackbar = {
            show: true,
            status: r.data.status,
