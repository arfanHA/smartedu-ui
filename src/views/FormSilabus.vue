...
export default {
  data() {
    return {
      ...
      tahunAjarId: null,
      editedItem: {
        moderation_modul_kategori_id: 1,
        pegawai_id: "",
        tahun_ajar_id: "",
        judul: null,
        file: null,
  ...
  methods: {
    ...
    fetchBahanAjarFile(myOffset) {
      ...
      this.$http
        .get("/api/moderation-modul/file", { params: params })
        .then((r) => {
          console.log(r);
    ...
    save() {
      ...
      if (this.$refs.form.validate() === true) {
        if (this.srcFile.file == null) {
          ...
          let fd = new FormData();
          fd.append(
            "moderation_modul_kategori_id",
            this.editedItem.moderation_modul_kategori_id
          );
          ...
          this.$store.commit("progressFunctionOn", true);
          this.$http
            .post("/api/moderation-modul/file", fd)
            .then((r) => {
              this.snackbar = {
                show: true,
    ...
    editItem(item) {
      ...
      this.editedItem.file = item.file;
    ...
    processEdit() {
      ...
      if (this.$refs.form.validate() === true) {
        if (this.srcFile.file == null) {
          ...
          this.$store.commit("progressFunctionOn", true);
          this.$http
            .post(`/api/moderation-modul/file/${this.editedItem.id}?_method=PUT`, fd)
            .then((r) => {
              this.snackbar = {
                show: true,
                status: r.data.status,
    ...
    processingDelete(item) {
      this.$store.commit("progressFunctionOn", true);
      this.$http
        .delete(`/api/moderation-modul/file/${item.id}`)
        .then((r) => {
          this.snackbar = {
            show: true,
            status: r.data.status,
  ...
  created() {
    this.fetchBahanAjarFile(1);
    this.tahunAjarId = JSON.parse(localStorage.getItem("tahunAjar"));
    ...
    this.editedItem.tahun_ajar_id = this.tahunAjarId.id;
  },
};
</script>
